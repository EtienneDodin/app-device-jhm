<x-app-layout>
    ...
    <form action="{{ route('devices.update', $device) }}" method="POST" x-data="{
        ...
        <div class="flex flex-col gap-2 items-center">
            ...
            <input type="text" name="code" id="code" value="{{ $device->code }}"
        ...
        <div class="flex flex-col gap-2 items-center">
            ...
            <select name="type_id" id="type_id" class="w-56 border rounded-md border-anthracite-grey"
                ...
                @foreach ($types as $type)
                    <option value="{{ $type->id }}" data-accepts-number="{{ $type->accepts_number }}"
                        data-accepts-ip="{{ $type->accepts_ip }}" {{ $type->id == $device->type_id ? 'selected' : '' }}>
                        {{ $type->name }}
                    </option>
                @endforeach

                <option class="text-gray-500" value="" {{ !$device->type_id ? 'selected' : '' }}>Non défini
                </option>
            </select>
        ...
        <div class="flex flex-col gap-2 items-center">
            ...
            <select name="location_id" id="location_id" class="w-56 border rounded-md border-anthracite-grey">
                ...
                <option class="text-gray-500" value="" {{ !$device->location_id ? 'selected' : '' }}>Non défini
                </option>
            </select>
        </div>
        ...
        <div class="flex flex-col gap-2 items-center">
            ...
            <select name="service_id" id="service_id" class="w-56 border rounded-md border-anthracite-grey">
                @foreach ($services as $service)
                    <option value="{{ $service->id }}" {{ $service->id == $device->service_id ? 'selected' : '' }}>
                        {{ $service->name }}</option>
                @endforeach

                <option class="text-gray-500" value="" {{ !$device->service_id ? 'selected' : '' }}>Non défini
                </option>
            </select>
        </div>
        ...
        <div x-show="showNumberField" x-cloak class="flex flex-col gap-2 items-center">
            ...
            @if ($device->phone_number)
                <input type="text" name="phone_number" id="phone_number" maxlength="10"
                    value="{{ '0' . $device->phone_number }}" class="w-56 border rounded-md border-anthracite-grey">
            @else
                <input type="text" name="phone_number" id="phone_number" maxlength="10"
                    class="w-56 border rounded-md border-anthracite-grey">
            @endif
        ...
        <div x-show="showIPField" x-cloak class="flex flex-col gap-2 items-center">
            ...
            <input type="text" name="ip" id="ip" value="{{ $device->ip }}"
                class="w-56 border rounded-md border-anthracite-grey">
        ...
        <button type="submit"
            class="font-akazan text-center text-dark-blue hover:text-main-gray hover:bg-light-blue transition duration-200 ease-in-out border rounded py-1.5 mb-12 w-20">MODIFIER</button>
    </form>
