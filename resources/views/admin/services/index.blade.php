<x-app-layout>
    ...
    <div class="flex flex-col items-center">
        ...
        <a href="{{ route('services.create') }}" class="mt-4 mb-16 py-1.5 px-4 flex gap-2 items-center bg-white hover:bg-light-gray transition duration-200 ease-in-out border rounded-md border-main-gray">
        ...
        <div>
            <div class="bg-light-gray w-80 font-proxima flex">
                <h1 class="text-lg pl-4 py-4">Liste des services</h1>
            </div>

            @foreach ($services as $service)
            <div class="flex relative py-1 items-center border-b">

                <p class="px-4 py-3 rounded font-proxima">{{ $service->name }}</p>

                <div class="absolute -right-44 flex gap-6 items-center">
                    {{-- edit --}}
                    <a href="{{ route('services.edit', $service) }}" class="rounded-md">
                        <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {{-- Delete --}}
                    <form action="{{ route('services.destroy', $service) }}" method="POST" x-data="{ open: false }">
                        @method('DELETE')
                        @csrf
                        ...
                        <div x-show="open" x-transition x-cloak class="fixed left-0 top-0 w-full h-screen z-10 flex justify-center items-center">
                            ...
                            <div @click.outside="open = false" class="bg-main-gray flex flex-col p-24 items-center gap-24 rounded border shadow z-10 w-6/12">
                                <p class="font-semibold text-3xl">Êtes-vous sûr de vouloir supprimer ce service ?</p>
