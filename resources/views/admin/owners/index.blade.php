<x-app-layout>
    <x-slot name="header">
        ...
        <div class="flex flex-col items-center">
            ...
            <a href="{{ route('owners.create') }}" class="group mt-4 mb-16 py-1.5 px-4 flex gap-2 items-center bg-white hover:bg-light-gray transition duration-200 ease-in-out border rounded-md border-main-gray">
            ...
            <div class="px-10">
                <h1 class="bg-light-gray font-proxima text-lg w-96 p-4">Liste des utilisateurs</h1>
                @foreach ($owners as $owner)

                    <div class="flex relative py-1 items-center border-b">
                        <p class="px-4 py-3 rounded font-proxima">{{ $owner->name }}</p>
                        <div class="absolute -right-44 flex gap-6 items-center">
                            {{-- edit --}}
                            <a href="{{ route('owners.edit', $owner) }}" class="rounded-md">
                            ...
                            {{-- delete --}}
                            <form action="{{ route('owners.destroy', $owner) }}" method="POST" x-data="{ open: false }">
                                @method('DELETE')
                                @csrf
