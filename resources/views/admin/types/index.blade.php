<x-app-layout>
    ...
    <div class="flex flex-col items-center">
        {{-- Add new device type --}}
        <a href="{{ route('types.create') }}" class="mt-4 mb-16 py-1.5 px-4 flex gap-2 items-center bg-white hover:bg-light-gray transition duration-200 ease-in-out border rounded-md border-main-gray">
        ...
        <div>
            {{-- Titles --}}
            <div class="bg-light-gray font-proxima flex items-center">
                <h1 class="text-lg w-44 pl-4 py-4">Liste des types</h1>
                <p class="w-32 text-center">Numéro</p>
                <p class="w-32 text-center">Adresse IP</p>
            </div>

            @foreach ($types as $type)
                <div class="flex relative py-1 items-center border-b">
                    <p class="pl-4 rounded w-44 font-proxima py-3">{{ $type->name }}</p>
        
                    <div class="w-32 flex justify-center">
                        <input type="checkbox" class="opacity-65" disabled {{ $type->accepts_number ? 'checked' : '' }}>
                    </div>

                    <div class="w-32 flex justify-center">
                        <input type="checkbox" class="opacity-65" disabled {{ $type->accepts_ip ? 'checked' : '' }}>
                    </div>

                    <div class="absolute -right-44 flex gap-6 items-center">
                        {{-- Edit --}}
                        <a href="{{ route('types.edit', $type) }}" class="group flex gap-4 items-end">
                            <svg width="20px" height="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            {{-- <span class="invisible group-hover:visible transition duration-200 ease-in-out text-sm font-proxima">Modifier</span> --}}
                        </a>

                        {{-- Delete --}}
                        <form action="{{ route('types.destroy', $type) }}" method="POST" x-data="{ open: false }">
                            @method('DELETE')
                            @csrf
                            <button type="button" @click="open = true" class="rounded-md px-3 py-1.5 border decoration-main-gray text-gray-500 text-sm hover:bg-light-gray transition duration-100 ease-in">
                                <svg enable-background="new 0 0 40 40" width="20px" height="30px" version="1.1" viewBox="0 0 40 40" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                            ...
                            <div x-show="open" x-transition x-cloak class="fixed left-0 top-0 w-full h-screen z-10 flex justify-center items-center">
                                {{-- Modal --}}
                                <div @click.outside="open = false" class="bg-main-gray flex flex-col px-16 py-24 items-center gap-24 rounded border shadow z-10 w-6/12">
                                    <p class="font-semibold text-3xl">Êtes-vous sûr de vouloir supprimer ce type d'équipement ?</p>
