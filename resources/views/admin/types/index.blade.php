        ...
        <div>
            ...
            @foreach ($types as $type)
                <div class="flex relative py-1 items-center">
                    <p class="pl-4 rounded w-44 font-proxima border-b py-3">{{ $type->name }}</p>
        
                    <div class="w-32 flex justify-center">
                        <input type="checkbox" disabled {{ $type->accepts_number ? 'checked' : '' }}>
                    </div>

                    <div class="w-32 flex justify-center">
                        <input type="checkbox" disabled {{ $type->accepts_ip ? 'checked' : '' }}>
                    </div>
                    ...
                    <div class="absolute -right-44 flex gap-6 items-center">
                        ...
                        <form action="{{ route('types.destroy', $type) }}" method="POST" x-data="{ open: false }">
                            ...
                            {{-- Container --}}
                            <div x-show="open" x-transition x-cloak @click.outside="open = false" class="fixed left-0 top-20 h-5/6 flex justify-center">
                                {{-- Modal --}}
                                <div class="bg-main-gray flex flex-col py-24 items-center gap-24 rounded border shadow z-[1] w-8/12">
                                    <p class="font-semibold text-3xl">Êtes-vous sûr de vouloir supprimer cet équipement ?</p>
                                    <div class="flex gap-20">
                                        <button type="submit" class="rounded-md bg-main-orange hover:bg-[#FD5555] py-1.5 px-8">Oui</button>
